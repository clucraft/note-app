...
import type { OAuthCredentials } from './gdrive.service.js';
...
export function getOAuthCredentials(): OAuthCredentials {
  ...
  const encRefreshToken = getSetting('backup_gdrive_refresh_token');

  if (!encClientId || !encClientSecret || !encRefreshToken) {
    throw new Error('Google Drive OAuth credentials not configured');
  }

  return {
    clientId: decrypt(encClientId),
    clientSecret: decrypt(encClientSecret),
    refreshToken: decrypt(encRefreshToken),
  };
}
...
export function getBackupConfig(): BackupConfig {
  ...
  const hasClientCredentials = !!getSetting('backup_gdrive_client_id') && !!getSetting('backup_gdrive_client_secret');
  const hasOAuthCredentials = hasClientCredentials && !!getSetting('backup_gdrive_refresh_token');
...
export async function runBackup(): Promise<void> {
  ...
  await uploadBackup(credentials, folderId, fileName, zipBuffer);
  await enforceRetention(credentials, folderId, maxRetention);
...
export function startScheduler(): void {
  ...
  if (!config.enabled || !config.hasOAuthCredentials || !config.folderId) {
